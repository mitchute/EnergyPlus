...
\tightlist
\item
  Note during hour 7 of the summer design day that ``NORTH PERIMETER:Zone/Sys Sensible Heating Rate {[}W{]}'' is nonzero, but the heating coil is off and the DX cooling coil shows a load.~ Why?~ This report variable reports the impact of the system on the zone (not the zone's demand for heating or cooling), averaged over the hour.~ The system fan is scheduled on at 6 a.m., but the outside air dampers are closed.~ The zone is not warm enough from the night to require cooling, so the circulating fan heat warms the zone slightly for a portion of the hour until the zone temperature exceeds the cooling setpoint and the DX coil comes on for the remainder of the hour.~ If the economizer were active, this would not occur.
\end{itemize}
