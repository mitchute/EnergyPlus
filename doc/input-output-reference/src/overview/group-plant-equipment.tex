...
\emph{This variable is zero when the chiller is not producing cooling. At reporting frequencies longer than ``detailed,'' EnergyPlus reports the time-weighted average of the timestep COP values, including zero values while the chiller is off. This value is not the COP for the reporting period. Calculate the reporting-period COP by dividing summed evaporator cooling energy by summed electric or fuel input energy over the same period.}
...
This is the operating coefficient of performance for the chiller in cooling operation. At each timestep, it is calculated as Chiller Heater Evaporator Cooling Rate divided by Chiller Heater Cooling \textless{}Fuel Type\textgreater{} Rate. If the cooling fuel consumption rate is zero, this output variable is set to zero.

\emph{At reporting frequencies longer than ``detailed,'' EnergyPlus reports the time-weighted average of the timestep COP values, including zero values while the chiller is off. This value is not the COP for the reporting period. Calculate the reporting-period COP by dividing summed Chiller Heater Evaporator Cooling Energy by summed Chiller Heater Cooling \textless{}Fuel Type\textgreater{} Consumption over the same period.}
...
This is the operating coefficient of performance for the chiller in cooling operation. At each timestep, it is calculated as Chiller Heater Evaporator Cooling Rate divided by Chiller Heater Cooling Source Heat Transfer Rate. If the cooling source heat transfer rate is zero, this output variable is set to zero.

\emph{At reporting frequencies longer than ``detailed,'' EnergyPlus reports the time-weighted average of the timestep COP values, including zero values while the chiller is off. This value is not the COP for the reporting period. Calculate the reporting-period COP by dividing the reporting-period Chiller Heater Evaporator Cooling Rate by the reporting-period Chiller Heater Cooling Source Heat Transfer Rate.}
...
\paragraph{Heat Pump Heating COP {[}{]}} The operating coefficient of performance
(COP) of the heat pump in heating mode, calculated at each timestep.
At reporting frequencies longer than ``detailed,'' EnergyPlus reports the
time-weighted average of the timestep COP values, including zero values while
the heat pump is off. This value is not the COP for the reporting period. Calculate
the reporting-period heating COP by dividing summed Heat Pump Load Side Heat
Transfer Energy in Heating Mode by summed Heat Pump Electricity Energy in Heating
Mode over the same period.
...
at the higher speed level; if the heat pump is variable-speed, this value is the
interpolated EIR PLR modifier between those two speed levels.
\paragraph{Heat Pump Cooling COP {[}{]}} The operating coefficient of performance
(COP) of the heat pump in cooling mode, calculated at each timestep.
At reporting frequencies longer than ``detailed,'' EnergyPlus reports the
time-weighted average of the timestep COP values, including zero values while
the heat pump is off. This value is not the COP for the reporting period. Calculate
the reporting-period cooling COP by dividing summed Heat Pump Load Side Heat
Transfer Energy in Cooling Mode by summed Heat Pump Electricity Energy in Cooling
Mode over the same period.
...
\emph{At reporting frequencies longer than ``detailed,'' EnergyPlus reports the time-weighted average of the timestep COP values, including zero values while the chiller-heater is off. This value is not the COP for the reporting period. Calculate reporting-period COP from the corresponding summed cooling and false-load heat transfer energies divided by summed electricity energy over the same period.}
