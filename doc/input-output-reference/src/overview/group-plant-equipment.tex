...
\begin{equation}
{\mathrm{Power  =  FracFullLoadPower }}
    \cdot {\mathrm{ FullLoadPowerRat }}\cdot {\mathrm{ AvailToNominalCapacityRatio }}\cdot \frac{{{\mathrm{NominalCapacity}}}}{{{\mathrm{COP}}}}
\end{equation}
...
\begin{equation}
{\Delta_{{\mathrm{Temp}}}}
    = \frac{{{\mathrm{TempCondIn  -  TempCondInDesign }}}}{{{\mathrm{TempRiseCoefficient}}}} - ({\mathrm{TempEvapOut  -  TempEvapOutDesign)}}
\end{equation}
...
\begin{equation}
    \begin{array}{l}
        \mathrm{CoolFuelInput} = \\
        \mathrm{AvailCoolCap} \cdot \mathrm{RunFrac} \cdot \mathrm{CFIR} \cdot \mathrm{CFIRfT}( T_{cw,l}, {T_{\mathrm{cond}}}) \cdot \mathrm{CFIRfPLR}( \mathrm{CPLR} )
    \end{array}
\end{equation}
...
\begin{equation}
    \begin{array}{l}
        \mathrm{CoolElectricPower} = \\
        \mathrm{NomCoolCap} \cdot \mathrm{RunFrac} \cdot \mathrm{CEIR} \cdot \mathrm{CEIRfT}(T_{cw,l}, T_{cond}) \cdot \mathrm{CEIRfPLR}(\mathrm{CPLR})
    \end{array}
\end{equation}
...
\begin{equation}
    \begin{array}{l}
        \mathrm{CoolThermalEnergyInput} = \mathrm{AvailCoolCap} \cdot \mathrm{RunFrac} \cdot \mathrm{TeFIR} \cdot \\
        \quad \quad \quad \quad \quad \quad \mathrm{TeFIRfT} \left( T_{cw,l}, T_{cond} \right) \cdot \mathrm{TeFIRfPLR}( \mathrm{CPLR} )
    \end{array}
\end{equation}
...
\begin{equation}
    \begin{array}{l}
        \mathrm{Bicubic} \to \mathrm{Eff} = A_0 + A_1 \cdot \mathrm{PLR} + A_2 \cdot \mathrm{PLR}^2 + A_3 \cdot T_w + A_4 T_w^2 + A_5 \mathrm{PLR} \cdot T_w + A_6 \mathrm{PLR}^3 \\
        \quad \quad \quad \quad \quad \quad \quad  + A_7 \cdot T_w^3 + A_8 \cdot \mathrm{PLR}^2 \cdot T_w + A_9 \cdot \mathrm{PLR} \cdot T_w^2
    \end{array}
\end{equation}
...
This output is the operating coefficient of performance for the boiler during heating operation. At each timestep, it is calculated by dividing Boiler Heating Rate by the sum of Boiler \textless{}Fuel Type\textgreater{} Rate, Boiler Ancillary Electricity Rate, and, for non-electric boilers, Boiler Ancillary \textless{}Fuel Type\textgreater{} Rate.

\emph{This variable is zero when the boiler is not producing heat. At reporting frequencies longer than ``detailed,'' EnergyPlus reports the time-weighted average of the timestep COP values, including zero values while the boiler is off. This value is not the COP for the reporting period. Calculate the reporting-period COP by dividing summed Boiler Heating Energy by the sum of Boiler \textless{}Fuel Type\textgreater{} Energy, Boiler Ancillary Electricity Energy, and, for non-electric boilers, Boiler Ancillary \textless{}Fuel Type\textgreater{} Energy over the same period.}
...
\paragraph{Boiler Steam Coefficient of Performance {[]}}\label{boiler-steam-coefficient-of-performance}

This output is the operating coefficient of performance for the steam boiler during heating operation. At each timestep, it is calculated by dividing Boiler Heating Rate by Boiler \textless{}Fuel Type\textgreater{} Rate.

\emph{This variable is zero when the steam boiler is not producing heat. At reporting frequencies longer than ``detailed,'' EnergyPlus reports the time-weighted average of the timestep COP values, including zero values while the boiler is off. This value is not the COP for the reporting period. Calculate the reporting-period COP by dividing summed Boiler Heating Energy by summed Boiler \textless{}Fuel Type\textgreater{} Energy over the same period.}
...
\subsubsection{Chiller Heater Operation Mode Unit \textless{}x\textgreater{}}\label{chiller-heater-operation-mode-unit-x}
...
\subsubsection{Chiller Heater Part Load Ratio Unit \textless{}x\textgreater{}}\label{chiller-heater-part-load-ratio-unit-x}
...
\subsubsection{Chiller Heater Cycling Ratio Unit \textless{}x\textgreater{}}\label{chiller-heater-cycling-ratio-unit-x}
