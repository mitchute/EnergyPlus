...
\begin{equation}
\mathrm{GroundReflectedSolar} = \left( \mathrm{BeamSolar} \cdot cos \left( \mathrm{SunZenithAngle} \right) + \mathrm{DiffuseSolar} \right) \cdot \mathrm{GroundReflectance}
\end{equation}
...
\begin{equation}
\mathrm{GroundReflectance}_{\mathrm{used}} = \mathrm{GroundReflectance} \cdot \mathrm{Modifier}_{\mathrm{Snow}}
\end{equation}
...
\begin{equation}
\mathrm{DaylightingGroundReflectance}_{\mathrm{used}} = \mathrm{GroundReflectance} \cdot \mathrm{Modifier}_{\mathrm{Snow}}
\end{equation}
...
If the calculation method is Correlation or CorrelationFromWeatherFile, the field further adjusts the calculated water mains temperature using the equation below. The default value is one, which means no adjustment. If the calculation method is Schedule, this field is ignored.

\begin{equation}
{T_{final}} = {T_{calculated}} * {Multiplier} + {Offset}
\end{equation}

\paragraph{Field: Temperature Offset}\label{field-temperature-offset}

If the calculation method is Correlation or CorrelationFromWeatherFile, the field {[}C{]} further adjusts the calculated water mains temperature using the equation below. The default value is zero, which means no offset. If the calculation method is Schedule, this field is ignored.
...
\begin{equation}
\mathrm{Amount} \, (m/hour) = \, \left\{
  \begin{array}{ccc}
    \mathrm{Light} = 0.0125 \\
    \mathrm{Moderate} = 0.052 \\
    \mathrm{Heavy} = 0.1
  \end{array}
\right.
\end{equation}
...
.epw is accurate. This \href{https://www.ncdc.noaa.gov/extremes/scec/records/all/maxp}{table by the State Climate Extreme
...
\begin{multline}
  \mathrm{Groundreflectedsolar}
  \\
  = \left( \mathrm{Beamsolar} \cdot cos \left( \mathrm{SolarAltitudeAngle} \right) + \mathrm{Diffusesolar} \right) \cdot \mathrm{Groundreflectance}_{\mathrm{month}}
\end{multline}
