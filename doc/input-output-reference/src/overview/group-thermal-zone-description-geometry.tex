...
\item
  Zone,Average,Zone Standard Mean Radiant Temperature {[}C{]}
\item
...
\paragraph{Zone Standard Mean Radiant Temperature {[}C{]}}\label{zone-standard-mean-radiant-temperature-c}
...
The "Standard" Mean Radiant Temperature (MRT) in degrees Celsius is the surface area $\times$ emissivity weighted average of the \hyperref[surface-inside-face-temperature-c]{Surface Inside Face Temperature}, where emissivity is the Thermal Absorptance of the inside material layer of each surface. Standard Zone Mean Radiant Temperature is based on all surfaces in the zone (regardless of what enclosure each surface is in). Enclosure Mean Radiant Temperature is based on all surfaces in an enclosure (one or more \hyperref[space]{Spaces}, see \hyperref[constructionairboundary]{Construction:AirBoundary}). Space Mean Radiant Temperature is based on all surfaces in the enclosure which contains the space (same as its Enclsoure Mean Radiant Temperature).  The Zone Mean Radiant Temperature is identical to the Standard Mean Radiant temperature except when the user inputs the \hyperref[zonemrtcalculation]{ZoneMRTCalculation} input object which determines how the zone MRT is calculated via People object input. When the ZoneMRTCalculation object is entered, the information entered via this object controls how the zone MRT is calculated and this then impacts any system controlled on either the MRT or the Operative Temperature (see next paragraph).
...
Zone,Average,Surface Thermal Absorptance []
Zone,Average,Surface Solar Absorptance []
Zone,Average,Surface Thermal Absorptance Outside Face []
Zone,Average,Surface Solar Absorptance Outside Face []
Zone,Average,Surface Thermal Absorptance Inside Face []
Zone,Average,Surface Solar Absorptance Inside Face []
\end{lstlisting}
...
The following variables apply only to opaque surfaces, where an opaque surface is considered here to be an exterior or interzone heat transfer surface of class FLOOR, WALL, ROOF or DOOR.
...
\subsubsection{Surface Thermal Absorptance {[}{]}}\label{surface-thermal-absorptance}

This variable reports the current outside-face thermal absorptance under the generic/bulk name. With no outside-face variable control, this is the bulk material value. If an outside-face control is active while no independent inside-face value or control applies, this variable follows the controlled outside-face value. When an explicitly entered inside-face thermal absorptance or an active inside-face thermal control applies, this generic variable is not registered for that surface; the corresponding Outside Face and Inside Face variables are registered instead.

\subsubsection{Surface Solar Absorptance {[}{]}}\label{surface-solar-absorptance}

This variable reports the current outside-face solar absorptance under the generic/bulk name. With no outside-face variable control, this is the bulk material value. If an outside-face control is active while no independent inside-face value or control applies, this variable follows the controlled outside-face value. When an explicitly entered inside-face solar absorptance or an active inside-face solar control applies, this generic variable is not registered for that surface; the corresponding Outside Face and Inside Face variables are registered instead.

Output availability is determined independently for each surface and for thermal and solar absorptance. Therefore, a wildcard Output:Variable request may return generic keys for some surfaces and face-specific keys for others. An explicitly entered inside-face value selects the face-specific pair even when it numerically equals the bulk value. An inside-face value or control on a material that is not the innermost construction layer does not change the output names for that surface. Inside-face variable controls are ignored for non-exterior surfaces.

The four face-specific variables below are registered as an outside/inside pair for a property only when the surface uses an explicitly entered inside-face value or an active inside-face variable-absorptance control for that property. Request both the generic and face-specific names when a single wildcard request must cover surfaces with both input configurations; EnergyPlus reports only the names applicable to each surface key.

\subsubsection{Surface Thermal Absorptance Outside Face {[}{]}}\label{surface-thermal-absorptance-outside-face}

This variable reports the current outside-face thermal absorptance. The value is taken from the outermost construction layer and may be constant or may vary according to an applicable \hyperref[materialpropertyvariableabsorptance]{MaterialProperty:VariableAbsorptance} control. See \hyperref[material]{Material} and \hyperref[materialnomass]{Material:NoMass} for the constant input fields.

\subsubsection{Surface Solar Absorptance Outside Face {[}{]}}\label{surface-solar-absorptance-outside-face}

This variable reports the current outside-face solar absorptance. The value is taken from the outermost construction layer and may be constant or may vary according to an applicable \hyperref[materialpropertyvariableabsorptance]{MaterialProperty:VariableAbsorptance} control. See \hyperref[material]{Material} and \hyperref[materialnomass]{Material:NoMass} for the constant input fields.

\subsubsection{Surface Thermal Absorptance Inside Face {[}{]}}\label{surface-thermal-absorptance-inside-face}

This variable reports the current inside-face thermal absorptance. The value is taken from the innermost construction layer and may be constant or may vary according to an applicable \hyperref[materialpropertyvariableabsorptance]{MaterialProperty:VariableAbsorptance} inside-face control. See \hyperref[material]{Material} and \hyperref[materialnomass]{Material:NoMass} for the constant input fields.

\subsubsection{Surface Solar Absorptance Inside Face {[}{]}}\label{surface-solar-absorptance-inside-face}

This variable reports the current inside-face solar absorptance. The value is taken from the innermost construction layer and may be constant or may vary according to an applicable \hyperref[materialpropertyvariableabsorptance]{MaterialProperty:VariableAbsorptance} inside-face control. See \hyperref[material]{Material} and \hyperref[materialnomass]{Material:NoMass} for the constant input fields.
