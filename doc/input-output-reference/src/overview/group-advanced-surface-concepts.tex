...
NREL/PR-5K00-72589. https://www.nlr.gov/docs/fy20osti/72589.pdf
...
\subsection{ZoneMRTCalculation}\label{zonemrtcalculation}

EnergyPlus can calculate different mean radiant temperatures (MRTs) for \hyperref[people]{People} objects. The \hyperref[field-mean-radiant-temperature-calculation-type]{Mean Radiant Temperature Calculation Type} field can select the standard enclosure-averaged MRT, a surface-weighted MRT, or an angle-factor MRT. ZoneMRTCalculation combines one or more of these People MRT values to define the MRT for one zone. This changes the zone MRT and zone operative temperature used for reporting and controls; it does not perform a separate space MRT calculation.

Only one ZoneMRTCalculation object is allowed for each zone. Each referenced People instance must belong to that zone, although the People object may be assigned directly to the zone or to an explicitly defined space within the zone. Referencing a space-assigned People object uses that People object's MRT calculation as a contribution to the enclosing zone MRT; it does not calculate or store an MRT for the space.

The ZoneMRTCalculation input object contains a zone name followed by an extensible list of People names and MRT weighting factors. Each weighting factor is a temperature weighting factor only. Referencing a People object here does not add occupants or duplicate that object's internal heat gains.

The sum of the weighting factors must be from 0.0 through 1.0. If the sum is less than 1.0, the remaining portion uses the standard zone MRT, which is an area-emissivity-weighted average of the zone surface temperatures. EnergyPlus issues a warning when the sum is less than 1.0 to make this fallback explicit. If the sum is greater than 1.0, EnergyPlus issues a severe error and resets all People weighting factors for this object to zero. The calculation is:

\begin{align}
\begin{split}
ZoneMRT = (fracPeople1)(People1MRT) + (fracPeople2)(People2MRT) + ... \\
        + (1.0-sumFracAllPeople)stdMRT
\end{split}
\end{align}

\subsubsection{Inputs}\label{inputs-zonemrtcalculation}

The input fields for this object are defined below.

\paragraph{Field: Zone Name}\label{field-zonemrtcalculation-zonename}

This field specifies the zone whose MRT will be calculated. It must name a Zone defined in the input file. Space and SpaceList names are not supported.

\paragraph{Field: Pair \textless{}\#\textgreater{} People Statement Name}\label{field-zonemrtcalculation-peoplenameX}

This field specifies the People MRT that will contribute to the zone MRT calculation. In conjunction with the next field, it defines a fraction of the MRT for the zone named above. Every referenced People instance must belong to that zone. A People object assigned directly to an explicitly defined space is supported when that space belongs to the named zone, and it retains the name entered in the input file.

When a People object is assigned to a zone that contains multiple spaces, EnergyPlus expands it into one People instance for each applicable space and changes each instance name to \texttt{<Space Name> <People Name>}. For example, a People object named \texttt{Zone X People} assigned to \texttt{Zone X}, which contains \texttt{Space X1} and \texttt{Space X2}, produces instances named \texttt{Space X1 Zone X People} and \texttt{Space X2 Zone X People}. People objects assigned to a ZoneList or SpaceList are expanded in the same way. ZoneMRTCalculation must reference one of these expanded instance names; the original People input name is not supported after expansion. People instances associated with automatically generated remainder spaces are also not supported.

Each referenced People object must select at least one \hyperref[field-thermal-comfort-model-type-up-to-7-allowed]{Thermal Comfort Model Type}. The selected model activates calculation of that People object's MRT method. Selecting more than one model does not change the MRT value used by ZoneMRTCalculation.

\paragraph{Field: Pair \textless{}\#\textgreater{} MRT Weighting Factor}\label{field-zonemrtcalculation-weightingfactorX}
...
In this example, each People object is assigned to \texttt{West Wing Zone}, and the zone does not contain multiple spaces.

If \texttt{West Wing People} is assigned to \texttt{West Wing Zone} and EnergyPlus expands it across the explicitly defined spaces \texttt{Oval Office}, \texttt{Staff Office}, and \texttt{Conference Room}, the following object is valid:

\begin{lstlisting}
ZoneMRTCalculation,
  West Wing Zone,                   !- Zone Name
  Oval Office West Wing People,     !- People 1 Name
  0.4,
  Staff Office West Wing People,    !- People 2 Name
  0.4,
  Conference Room West Wing People,  !- People 3 Name
  0.2;
\end{lstlisting}
