...
\begin{equation}
\frac{{\varphi \left( {{x_{fan}}} \right)}}{{{\varphi_{\max }}}} = ({A_{spd}} + {B_{spd}}/({(1 + {e^{\left[ {{\frac{{({C_{spd}} - {x_{spd}})}}{{D_{spd}}}}} \right]}})^{{E_{spd}}}})
...
Fan shaft torque (N\(\cdot\)m), which the belt drive must supply to the fan shaft, is then:
