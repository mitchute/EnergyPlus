...
\begin{equation}
C_{Shape} = \left\{ \begin{array}{l}0.754\;\;\;{\mathrm{for}}\;{\mathrm{a}}\;{\mathrm{circle}}\;{\mathrm{opening}}\\0.942(w/D)\;{\mathrm{for}}\;{\mathrm{a}}\;{\mathrm{rectangle}}\;{\mathrm{opening}}\end{array} \right\}
\end{equation}
...
$A_\textrm{in}$ and exits through another of equal area. Let us denote the flow rate
of ambient air into the room by $Q$, and define a dimensionless flow rate $Q'$ by
\begin{equation}
Q'=Q/(A_\textrm{in} U_\textrm{ref}),
\end{equation}
where $U_\textrm{ref}$ is a reference velocity taken as the undisturbed wind speed
$U(z_\textrm{ref})$ at a reference height $z_\textrm{ref} = 10 \textrm{m}$. There is a
...
\begin{equation}
\Delta c_p=\frac{\overline{\Delta p}}{\frac{1}{2} \rho U_\textrm{ref}^2},
\end{equation}
\begin{equation}
\sigma_{\Delta c_p}=\frac{\sigma_{\Delta p}}{\frac{1}{2} \rho U_\textrm{ref}^2},
\end{equation}
$\rho$ being the ambient air density and
\begin{equation}
U_L'=\frac{U_L}{U_\textrm{ref}}.
\end{equation}
...
\end{equation}
where the dimensionless separation $s' = s/\textrm{WB}$,
\begin{equation}
...
\begin{equation}
T_{comf} = \left\{ \begin{array}{ll} \mathrm{Low Temp Curve,} & \min T_{out} < x \leq \mathrm{Boundary Point} \\ \mathrm{High Temp Curve,} & \mathrm{Boundary Point} \leq x < \max T_{out}  \end{array}\right.
\end{equation}
...
\begin{equation}
\theta = -0.0028 \left(100-\mathrm{PPD}\right)^2 + 0.3419 \left(100-\mathrm{PPD}\right) - 6.6275
\end{equation}
...
\begin{equation}
T_g > \left(T_{comf} + \theta\right)~~\&~~\mathrm{OpeningProbability}
\end{equation}
...
\begin{equation}
T_g < \left(T_{comf} - \theta\right)~~\&~~\mathrm{ClosingProbability}
\end{equation}
...
When \(\Delta P\) is given, and the relationship between D and V is also given, there is only a single unknown D. The value can be obtained through iteration.
