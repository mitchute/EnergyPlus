...
\text{Where: }\\
  ...
  \Sigma{\dot{m}_{zi}(W_{zi}-W_z)} &: \text{Moisture transfer due to interzone air mixing} ~ [kg/s],\\
...
\begin{equation}
q_{inf}= \frac {\dot{m}_{inf}} {\rho_{air}}
\end{equation}
...
\begin{align}
  ...
  &-  [\Sigma{h_i A_i (T_{si}-T_z)} + \Sigma{\dot{m}_{zi}C_p(T_{zi}-T_z)} + \dot{m}_{inf}C_p(T_o - T_z) + \dot{m}_{sys}C_p(T_{sys} - T_z)]
\end{align}
...
\begin{align}
  ...
  &-  [\Sigma{A_i h_i \rho_{air} (W_{si} - W_{z}^{t})} + \Sigma{\dot{m}_{zi} C_p (W_{zi}-W_{z}^{t})}]
\end{align}
...
\begin{align}
  ...
  &-  [\Sigma{\dot{m}_{zi}(C_{zi}-C_z)} \dot{m}_{inf} (C_o - C_z) + \dot{m}_{sys} (C_{sys} - C_z)]
\end{align}
