...
\ifdefined\pandoc
Air mass 1.5 terrestrial solar global spectral irradiance values (W/m-micron) on a & & & & & & & & & \\
37 degree tilted surface.~Corresponds to wavelengths in following data block. & & & & & & & & & \\
Based on ISO 9845-1 and ASTM E 892;~derived from Optics5 data file & & & & & & & & & \\
ISO-9845GlobalNorm.std, 10-14-99. & & & & & & & & & \\
\fi
\hline
\hfill 0.0, & 9.5, & 42.3, & 107.8, & 181.0, & 246.0, & 395.3, & 390.1, & 435.3, & 438.9, \\
...
\fi
\hline
\hfill 0.3000, & 0.3050, & 0.3100, & 0.3150, & 0.3200, & 0.3250, & 0.3300, & 0.3350, & 0.3400, & 0.3450, \\
...
\ifdefined\pandoc
Photopic response function values corresponding to wavelengths in following data block. & & & & & & & & & \\
Based on CIE 1931 observer; ISO/CIE 10527, CIE Standard Calorimetric Observers; & & & & & & & & & \\
derived from Optics5 data file "CIE 1931 Color Match from E308.txt", & & & & & & & & & \\
which is the same as WINDOW4 file Cie31t.dat. & & & & & & & & & \\
\fi
\hline
\hfill 0.0000, & 0.0001, & 0.0001, & 0.0002, & 0.0004, & 0.0006, & 0.0012, & 0.0022, & 0.0040, & 0.0073, \\
...
\fi
\hline
\hfill .380, & .385, & .390, & .395, & .400, & .405, & .410, & .415, & .420, & .425, \\
...
The following discussion assumes that optical quantities such as transmissivity, reflectivity, absorptivity, and index of refraction are a function of wavelength, \(\lambda\). If there are no spectral data the angular dependence is calculated based on the single values for transmittance and reflectance in the visible and solar range. In the visible range an average wavelength of 0.575 microns is used in the calculations. In the solar range an average wavelength of 0.898 microns is used.
...
Given the diffuse reflectance \(\rho_{sc}\) and the screen aspect ratio \(\gamma\), the model takes the direction of solar incidence, the relative solar altitude angle \(\alpha'\) and the relative solar azimuth angle \(\phi'\), illustrated in Figure~\ref{fig:schematic-of-a-vertical-window-screen-facing}, and calculates the direct beam transmittance \(T_{beam}\) (\(\alpha'\), \(\varphi'\)) as follows. Since the direct beam transmittance is only a function of the incident angle and the screen material aspect ratio, the following applies to both solar and visible radiation.
...
\begin{equation}
\begin{array}{ccc}{{\boldsymbol{\Lambda}^{{\mathrm{(s)}}}} = \left( {{\Lambda ^{{\mathrm{(s)}}}}_{ij}} \right)}&{{\mathrm{where}}}&{\Lambda_{ij}^{{\mathrm{(s)}}} = \left( {{{\mathbf{p}}_j}\cdot {{\mathbf{n}}^{{\mathrm{(s)}}}}} \right)\Delta {\Omega_j}{\delta_{ij}}}\end{array}
...
\begin{equation}
{{\mathbf{E}}^{{\mathrm{(I)}}}}{\mathbf{ = }}{\boldsymbol{\Lambda}^{{\mathrm{(I)}}}} \cdot {{\mathbf{S}}^{{\mathrm{(I)}}}}
\end{equation}

\begin{equation}
{{\mathbf{S}}^{{\mathrm{(T)}}}}{\mathbf{ = T}} \cdot {\boldsymbol{\Lambda}^{{\mathrm{(I)}}}} \cdot {{\mathbf{S}}^{{\mathrm{(I)}}}}
\end{equation}
...
\(\Lambda_{k,j}\) = lambda value of exterior window i at incidence direction k for outgoing direction j
