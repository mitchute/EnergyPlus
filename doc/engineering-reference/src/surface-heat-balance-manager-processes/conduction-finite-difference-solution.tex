...
\begin{equation}
T_{\text{$i$,new}} = T_{\text{$i$,old}}
                         + \text{Relax}\left(T_{\text{$i$,new}}
                            - T_{\text{$i$,old}} \right)
\end{equation}
...
\begin{equation}
Cp = \frac{h_{\text{$i$,new}} - h_{\text{$i$,old}}}
          {T_{\text{$i$,new}} - T_{\text{$i$,old}}}
\end{equation}
...
\begin{equation}
Cp = f\left(T_{\text{$i$,new}}, T_{\text{$i$, prev}},
            \text{PhaseState}_{\text{new}},
            \text{PhaseState}_{\text{prev}}\right)
\end{equation}
...
where:
\begin{description}
\item[\(k_o =\)] the \SI{20}{\celsius} value of thermal conductivity (normal IDF
input)
\item[\(k_1 =\)] the change in conductivity per degree temperature difference from
\SI{20}{\celsius}
\end{description}
...
+&\text{HeatCap1}_{i+1}\frac{T_{\text{$i+1$,new}} -
T_{\text{$i+1$,old}}}{\Delta t} \\
- \text{QSource}_{i}
+&\text{HeatCap2}_{i}
\frac{T_{\text{$i$,new}}-T_{\text{$i$,old}}}{\Delta t}
\end{split}
\end{align}
%
where:
...
\item[\(Q_{\text{inside}} =\)] Surface Inside Face Conduction Heat Transfer Rate
per Area {[}W/m\(^{2}\){]}
