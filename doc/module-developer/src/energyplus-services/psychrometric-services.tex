...
Returns the Latent energy of air \emph{Hfg} (joules per kilogram) as a function of humidity ratio {[}W{]} (kilograms of water per kilogram of dry air) and dry bulb temperature {[}Tdb{]} (Celsius).~ It calculates hg and then hf and the difference is Hfg.
...
Returns the Vapor Density in air \emph{RhoVapor} (kilograms of water per cubic meter of air) as a function of dry bulb temperature \emph{Tdb} (Celsius), Relative Humidity {[}Rh{]} (fraction).

\subsection{PsyRhovFnTdbWP (Tdb,W,Pb,calledfrom)}\label{psyrhovfntdbwp-tdbwpbcalledfrom}

Returns the Vapor Density in air \emph{RhoVapor} (kilograms of water per cubic meter of air) as a function of dry bulb temperature \emph{Tdb} (Celsius), humidity ratio {[}W{]} (kilograms of water per kilogram of dry air) and barometric pressure {[}Pb{]} (Pascals).

\subsection{PsyRhFnTdbRhov (Tdb,Rhov,calledfrom)}\label{psyrhfntdbrhov-tdbrhovcalledfrom}

Returns the Relative Humidity {[}Rh{]} (fraction) in air as a function of dry bulb temperature {[}Tdb{]} (Celsius) and Vapor Density in air \emph{RhoVapor} (kilograms of water per cubic meter of air).
